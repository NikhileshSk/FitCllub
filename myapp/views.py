...
def signup(request):
    if request.method == 'POST':
        ...
        try:
            ...
            user_profile = UserProfile(user=user, height=height, weight=weight, age=age, gender=gender, date_of_birth=date_of_birth)
            user_profile.save()
...
def counter(request):
        ...
        if request.method == 'POST':
            ...
            api_request = requests.get(api_url + query, headers={'X-Api-Key': '/F+R8ET6NYsnT0Vyx/5LiQ==DY668qUTqhB3MvaG'} )
            try:
            ...
            if flag == 'yes':
                ...
                return render(request,'counter.html',{'asdasd':'asdadasda'})   
        else:
